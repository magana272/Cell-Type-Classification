...
def plot_pca(ds: cell_dataset.GeneExpressionDataset,
             seed=42, n_components=50, save_path='/figure', file_name='fig_pca.png'):
...
def plot_heatmap(ds: cell_dataset.GeneExpressionDataset, gene_names,
    ...
    """Plot a clustered heatmap of the top-N highly variable genes across a balanced sample of cells per type."""
    X = _to_numpy(ds.X)
    labels = _string_labels(ds)
    class_names = _class_names(ds)
    gene_names = np.asarray(gene_names)
